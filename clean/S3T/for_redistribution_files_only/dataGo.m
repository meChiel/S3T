...
%     };
function dataGo(arguments)
%% Arguments processing
if ~exist('arguments','var')
    pdir=0;
    processSingleFile = 1;
else
    if (~isfield(arguments,'pdir'))
        pdir=0;
        processSingleFile = 1;
        
        if (~isfield(arguments,'fname'))
            fname=0;
...
for i=3:length(d) % 1,2 is \. and \..
    if d(i).isdir
        ...
        % Check for subdir in this subdir.
        d2 = dir([ pdir '\' d(i).name '/*.*']);
        for ii=3:length(d2) % 1,2 is \. and \..
            if d2(ii).isdir
                k=k+1;
                dirs{k}=[pdir '\' d(i).name '\' d2(ii).name];
            end
        end
    end
end
...
for i=1:length(dirs)
    ...
    if 1
        ...
        if 1
            if processSingleFile
                tiffilenames =[inputDir '\' arguments.fname '.tif'];
            else
   ...
   %         [U,S,V,FLAG] = svds(reshape(frames,wx*wy,[]),6);
            ...
            for iii=1:36
                plot(V(:,iii));
                saveas(fig,[sourceName 'eigenNeuron_' num2str(iii) 'R.png']);
            end
            plot(V);
