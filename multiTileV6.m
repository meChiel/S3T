...
fname= 'F:\share\data\Michiel\photoshoot\stimx40-w29\NS_60_20190514_164245\NS_60_m000000_tt0000.tif';
txtname  = 'F:\share\data\Michiel\photoshoot\stimx40-w29\NS_60_20190514_164245\NS_60.txt';

%%

fname1= 'F:\share\data\Michiel\photoshoot\stimx60-w29\NS_66.tif';
txtname  = 'F:\share\data\Michiel\photoshoot\stimx60-w29\NS_66.txt';
...
switch '40x'
    ...
    case '40x'        
        ...
        scy=1/0.407;
     case '60x'
        scx=1/0.266666*0.9657;
        scy=1/0.266666*0.9657;
end
   rsc=4; %rescale factor
A=zeros(ceil(dy*scy/rsc)+512+512,ceil(dx*scx/rsc)+512+512,20);
...
for k = 1:1681%100%1681/4
    k
    if 1 %read each file separatly
        PathName = 'F:\share\data\Michiel\photoshoot\NS_920190507_162402_20190507_172619';
        PathName = 'F:\share\data\Michiel\photoshoot\stimx40-w29\NS_60_20190514_164245';
        ppp='NS_920190507_162402_m';% 001679;% '_tt0003';
         ppp='NS_60_m';
        FileName=[ num2str(1000000+k-1)];
    ...
    else %read each frames from 1 databasefile
        C=[];
...
%         S=1;
    ...
    for ii=1:20%3
        %B=reshape(U(:,ii)*S(ii,ii)*V(:,ii)',[512,512]);
     
        B=imresize(reshape(C(:,ii),[512,512]),1/rsc);
    ...
    cy=floor((pR(2)-my)*scy/rsc);
    cx=floor((pR(1)-mx)*scx/rsc);
    cx
...
if writeMovie
    %vidObj = VideoWriter([dd '\' fname(end).name '__overviewVideo_a' num2str(accuracy) '.avi']);
      vidObj = VideoWriter([fname '__overviewVideo_a' num2str(accuracy) '.avi']);
...
for i=1:20
    currFrame=A(:,:,i)/109;
         writeVideo(vidObj,currFrame);
                disp(['writing frame ' num2str(i) ' of ' num2str(size(V,1))]);
end 
                  if writeMovie
        disp(['created: ' fname '__overviewVideo_a' num2str(accuracy) '.avi']);
        close(vidObj);
    end
